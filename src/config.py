...
IMAGE_SIZE = 224  # Input image size (images are resized to IMAGE_SIZE x IMAGE_SIZE).
...
MODEL_TRAINING_CONFIGS = {
    "SimpleCNN": {
        "lr": 2e-4,  # Single LR because model has no pretrained backbone split.
        "freeze_backbone_epochs": 0,  # No backbone freezing for scratch CNN.
    },
    "ResNet18": {
        "backbone_lr": 5e-5,  # Lower LR for transferred feature extractor.
        "head_lr": 3e-4,  # Higher LR for task-specific classifier head.
...
SCHEDULER_MIN_LR = 1e-7  # Lowest LR floor after scheduler decay.
SCHEDULER_WARMUP_EPOCHS = 1  # Number of linear warmup epochs before cosine decay.
SCHEDULER_WARMUP_START_FACTOR = 0.3  # Initial warmup LR factor relative to base LR.
...
BEST_MODEL_NAME = "best_simple_cnn.pt"  # Default best-model filename.
